...
def chooseTeam():
    ...
    for i in range(team_length):
        ...
        team_member = copy.copy(get_pokemon_data(selection))
        print(f"Chose {team_member.name}!")
...
def startBattle():
    ...
    while team_size > 0:
        pokeId = randint(0, POKEDEX_MAX) + 1
        mon = copy.copy(get_pokemon_data(pokeId))
...
def playerSwitch(Player, forced):
    ...
    # opponent wins
    if all(pokemon.battleStats['hp'] <= 0 for pokemon in Player.team):
        endBattle(opponent)
    for k in range(len(Player.team)):
    ...
    if Player.team[mon] == Player.current_pokemon or Player.team[mon].battleStats['hp'] <= 0:
...
def changeStats(move, Player, opponent, user):
    print("Working on it. Go fuck yourself")
    return
...
#     for i in move.stat_modifier:
#         if move.stat_modifier[i] < 0:
#             change = "decreased"
#         else:
#             change = "increased"
#         if move.stat_modifier[i] > 1 or move.stat_modifier[i] < -1:
#             changeIntensity = " drastically"
#             punctuation = "!"
#         # this is where the actual number crunching happens
#         statArray[user][move.stat[i]] = statArray[user][move.stat[i]] + move.stat_modifier[i]
#         # if the stat is going higher than 6, don't allow it
#         if statArray[user][move.stat[i]] > 6:
#             print(
#                 f"{userObject.name}'s {userObject.current_pokemon.name}'s {statNameArray[move.stat[i]]} cannot go any higher")
#             statArray[user][move.stat[i]] = 6
#         elif statArray[user][move.stat[i]] < -6:
#             print(f"{userObject.name}'s {userObject.current_pokemon.name}'s {statNameArray[move.stat[i]]} cannot go any lower")
#             statArray[user][move.stat[i]] = -6
#         # otherwise, indicate the stat change
#         else:
#             print(
#                 f"{userObject.name}'s {userObject.current_pokemon.name}'s {statNameArray[move.stat[i]]}"
...
def playerHealthCheck(Player, opponent):
    ...
    if Player.current_pokemon.battleStats['hp'] <= 0:
...
def oppHealthCheck(opponent):
    ...
    if opponent.current_pokemon.battleStats['hp'] <= 0:
...
def generateHealthBars(Player, opponent):
    ...
    # same for opponent
    hashes1 = int(opponent.current_pokemon.battleStats['hp'] / hpMultiple)
    spaces1 = int((opponent.current_pokemon.pokeStats["hp"] - opponent.current_pokemon.battleStats['hp']) / hpMultiple)
    hashes1 = str(hashes1 * "#")
    spaces1 = str(spaces1 * "-")
...
def getInfo(Pokemon):
    ...
    # print out the stats
    print(f"HP is: {Pokemon.battleStats['hp']}/{Pokemon.pokeStats['hp']}")
    print(f"Attack is: {Pokemon.battleStats['attack']}")
    print(f"Defense is: {Pokemon.battleStats['defense']}")
    print(f"Special Attack is: {Pokemon.battleStats['special-attack']}")
    print(f"Special Defense is: {Pokemon.battleStats['special-defense']}")
    print(f"Speed is: {Pokemon.battleStats['speed']}")
    return
...
def Turn(Player, opponent):
    ...
    while Player.current_pokemon.battleStats['hp'] > 0 and opponent.current_pokemon.battleStats['hp'] > 0:
        ...
        if battle_choice == 1:
            ...
            for i in range(len(Player.current_pokemon.moves)):
                print(f"{str(i + 1)}. {Player.current_pokemon.moves[i].name} "
                      f"Type: {Player.current_pokemon.moves[i].move_type} "
